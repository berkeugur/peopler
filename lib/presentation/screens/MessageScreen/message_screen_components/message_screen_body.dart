...
class MessageScreenBody extends StatelessWidget {
  ...
  _showMessages() {
    ...
    return GroupedListView<dynamic, String>(
        ...
        indexedItemBuilder: (context, t, index) {
          bool _isNextItem =
              _messages[index + (_messages.length - 1 != index ? 1 : 0)].from == _messages[index].from ? true : false;
  ...
  bool _listScrollListener(BuildContext context, double loadMoreOffset) {
    ...
    if (_messageBloc.isScrolling == false) {
      ...
      if (messageListController.position.pixels <= loadMoreOffset &&
          messageListController.position.userScrollDirection == ScrollDirection.forward) {
                      ...
                      child: Text(
                        value.toString().substring(0, 10) == _now.toString().substring(0, 10)
                            ...
                            : value.toString().substring(0, 10) ==
                                    _now.subtract(const Duration(days: 1)).toString().substring(0, 10)
                                ? "Dün"
